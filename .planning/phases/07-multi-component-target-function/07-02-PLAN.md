...
- `test_custom_weights`: `SAParams(formula="C6H14", component_weights={"wiener_index": 0.7, "molecular_weight": 0.3})` succeeds
...
- `test_two_components_combined`: SAParams with `component_weights={"wiener_index": 1.0, "molecular_weight": 1.0}` produces initial_energy that is approximately `35 + 86.18` (within tolerance of 0.5). Use `pytest.approx()`.
- `test_zero_weight_component_excluded`: SAParams with `component_weights={"wiener_index": 1.0, "molecular_weight": 0.0}` produces `initial_energy == 35` (MW has zero weight, so only Wiener contributes).
...
- `test_unknown_component_lists_available`: The ValueError message from above includes "wiener_index" and "molecular_weight".
...
- `test_run_completes_with_multi_component`: Full run with `{"wiener_index": 0.5, "molecular_weight": 0.5}`, 100 steps x 2 cycles, seed=42. Verify: result has valid SMILES, accounting sums to total, acceptance_ratio in [0, 1].
...
7. SAEngine with `{"wiener_index": 1.0, "molecular_weight": 1.0}` on C6H14 produces `initial_energy ~= 121.18` (35 + 86.18)
</verification>
