...
must_haves:
  truths:
    ...
    - "LogPComponent.compute() returns correct Wildman-Crippen LogP via RDKit Descriptors.MolLogP"
  ...
  artifacts:
    ...
    - path: "backend/app/core/scoring/logp.py"
      provides: "LogPComponent class satisfying ScoringComponent protocol"
      contains: "class LogPComponent"
...
<objective>
Create the scoring component framework: Protocol interface, two concrete components (Wiener Index and LogP), and a component registry with validation.
...
NOTE: MolecularWeight was replaced with LogP because all constitutional isomers of a given formula have the same MW — it would never change during SA optimization. LogP (Wildman-Crippen) varies with atom connectivity and is practically relevant.
</objective>
...
**TestLogPComponent:**
- `test_name_is_logp`: assert `component.name == "logp"`
- `test_linear_vs_branched_logp_differs`: Create linear hexane (`create_linear_alkane(6)`) and branched isobutane (`create_branched('isobutane')`). Assert their LogP values differ (proves LogP varies across isomers, unlike MW).
- `test_hexane_logp_positive`: Create `MoleculeGraph.create_linear_alkane(6)`, assert `component.compute(graph) > 0` (hydrocarbons have positive LogP)
...
- `test_logp_has_name_and_compute`: same for LogPComponent

**TestComponentRegistry:**
- `test_default_registry_has_wiener`: `get_registry().get("wiener_index")` does not raise
- `test_default_registry_has_logp`: `get_registry().get("logp")` does not raise
...
- `test_unknown_component_error_lists_available`: The KeyError message contains "wiener_index" and "logp"
...
**3. `backend/app/core/scoring/logp.py`:**
...
Use RDKit's built-in `Descriptors.MolLogP()` -- do NOT hand-roll.
...
<success_criteria>
- `backend/app/core/scoring/` package exists with protocol.py, wiener.py, logp.py, registry.py
...
- LogPComponent uses `RDKit Descriptors.MolLogP()` and varies across constitutional isomers
